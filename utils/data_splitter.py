...
def perform_auto_split(config):
    if not config['auto_split']['enabled']:
        return config['train_dir'], config['val_dir'], config.get('test_dir')
    ...
    test_split = config['auto_split'].get('test_split', 0.0)  # default 0 se non specificato
    ...
    num_test_per_class = int(min_images * test_split)
    ...
    for class_dir in class_dirs:
        ...
        test_imgs = images[num_val_per_class:num_val_per_class + num_test_per_class]
        train_imgs = images[num_val_per_class + num_test_per_class:]

        for subset_dir, subset_imgs in [
            (train_dir, train_imgs), (val_dir, val_imgs), (test_dir, test_imgs)
        ]:
    ...
    print(f"Split completato: {num_val_per_class} val, {num_test_per_class} test immagini per classe.")
